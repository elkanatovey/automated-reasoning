...
def homework_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step7 = prover.add_tautological_implication('((Homework(x)&Reading(x))->(Reading(x)&~Fun(x)))',
                                                {step6})

    step8 = prover.add_instantiated_assumption('((Reading(x)&~Fun(x))->Ex[(Reading(x)&~Fun(x))])', prover.EI,
                                               {'R': '(Reading(_)&~Fun(_))', 'x': 'x', 'c': 'x'})
    step9 = prover.add_tautological_implication('((Homework(x)&Reading(x))->Ex[(Reading(x)&~Fun(x))])', {step7, step8})
...
def multiply_zero_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step1 = prover.add_free_instantiation('plus(minus(x),x)=0', negation, {})
    step2 = prover.add_flipped_equality('0=plus(minus(x),x)', step1)
    step3 = prover.add_free_instantiation('times(x,1)=x', mult_identity, {})
    step4 = prover.add_flipped_equality('x=times(x,1)', step3)
    step5 = prover.add_substituted_equality('plus(minus(x),x)=plus(minus(x),times(x,1))', step4, 'plus(minus(x),_)')
    step7 = prover.add_free_instantiation('plus(0,1)=1', zero, {'x': '1'})
    step8 = prover.add_flipped_equality('1=plus(0,1)', step7)
    step9 = prover.add_substituted_equality('plus(minus(x),times(x,1))=plus(minus(x),times(x,plus(0,1)))', step8,
                                            'plus(minus(x),times(x,_))')
    step10 = prover.add_free_instantiation('times(x,plus(0,1))=plus(times(x,0),times(x,1))', mult_distributivity,
                                           {'y': '0', 'z': '1'})
...
def russell_paradox_proof(print_as_proof_forms: bool = False) -> Proof:
    ...
    step2 = prover.add_instantiated_assumption(
        ...
        {'R': '((In(_,y)->~In(_,_))&(~In(_,_)->In(_,y)))', 'c': 'y'})
    step3 = prover.add_tautology('(((In(y,y)->~In(y,y))&(~In(y,y)->In(y,y)))->(z=z&~z=z))')
    step4 = prover.add_tautological_implication('(Ax[((In(x,y)->~In(x,x))&(~In(x,x)->In(x,y)))]->(z=z&~z=z))', {step2,
                                              step3})
    prover.add_existential_derivation('(z=z&~z=z)', step1, step4)
    return prover.qed()
