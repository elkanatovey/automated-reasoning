...
class Model(Generic[T]):
    ...
    def evaluate_term(self, term: Term,
        ...
        return self.evaluate_term_help(term, assignment)

    def evaluate_term_help(self, term: Term,
                           assignment: Mapping[str, T] = frozendict()) -> T:
        if is_variable(term.root):
            return assignment[term.root]
        elif is_constant(term.root):
            ...
            evaluation = list()
            for arg in term.arguments:
                evaluation.append(self.evaluate_term_help(arg, assignment))
            k = tuple(evaluation)
            return self.function_meanings[term.root][k]
    ...
    def evaluate_formula(self, formula: Formula,
        ...
        #         evaluation.append(self.evaluate_term(term, assignment))
        #     return tuple(evaluation) in self.relation_meanings[formula.root]
        #
        ...
        #                self.evaluate_formula(formula.second, assignment)
        #     if formula.root == '->':
        #         if self.evaluate_formula(formula.second, assignment):
        #             return True
        #         elif not self.evaluate_formula(formula.first, assignment):
        #             return True
        #         return False
        #
        # if is_quantifier(formula.root):
        #     if formula.root == 'A':
        #         copied_assignments = dict()
        #         copied_assignments.update(assignment)
        #         for item in self.universe:
        #             copied_assignments[formula.variable] = item
        #             # value of x in universe for which phi is false
        ...
        #         for item in self.universe:
        #             copied_assignments[formula.variable] = item
        #             # value of x in universe for which phi is true
        #             if self.evaluate_formula(formula.predicate,
        #                                          copied_assignments):
        #                 return True
        #         return False
        return self.evaluate_formula_help(formula, assignment)

    def evaluate_formula_help(self, formula: Formula,
                         assignment: Mapping[str, T] = frozendict()) -> bool:
        if is_equality(formula.root):
            return self.evaluate_term_help(formula.arguments[0], assignment) == \
                   self.evaluate_term_help(formula.arguments[1], assignment)

        if is_relation(formula.root):
            evaluation = list()
            for term in formula.arguments:
                evaluation.append(self.evaluate_term_help(term, assignment))
            return tuple(evaluation) in self.relation_meanings[formula.root]

        if is_unary(formula.root):
            return not self.evaluate_formula_help(formula.first, assignment)

        if is_binary(formula.root):
            if formula.root == '&':
                return self.evaluate_formula_help(formula.first, assignment) & \
                       self.evaluate_formula_help(formula.second, assignment)
            if formula.root == '|':
                return self.evaluate_formula_help(formula.first, assignment) | \
                       self.evaluate_formula_help(formula.second, assignment)
            if formula.root == '->':
                if self.evaluate_formula_help(formula.second, assignment):
                    return True
                elif not self.evaluate_formula_help(formula.first, assignment):
                    return True
                return False
        ...
        if is_quantifier(formula.root):
            if formula.root == 'A':
                ...
                for item in self.universe:
                    copied_assignments[formula.variable] = item
                    # value of x in universe for which phi is false
                    if not self.evaluate_formula_help(formula.predicate,
                                                 copied_assignments):
                        return False
                return True
            if formula.root == 'E':
                ...
                for item in self.universe:
                    copied_assignments[formula.variable] = item
                    # value of x in universe for which phi is true
                    if self.evaluate_formula_help(formula.predicate,
                                             copied_assignments):
                        return True
                return False
    ...
    def is_model_of(self, formulas: AbstractSet[Formula]) -> bool:
        ...
        #         return False
        # return True
        return self.is_model_of_help(formulas)

    def is_model_of_help(self, formulas: AbstractSet[Formula]) -> bool:
        for formula in formulas:
            free_vars = formula.free_variables()
            formula_to_check = formula
            for var in free_vars:
                formula_to_check = Formula('A', var, formula_to_check)
            if not self.evaluate_formula_help(formula_to_check,{}):
                return False
        return True
